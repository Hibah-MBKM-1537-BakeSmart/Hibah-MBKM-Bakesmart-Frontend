...
import { Users, UserCheck, Activity, MessageCircle } from 'lucide-react';
import { useCustomers } from '../../../app/contexts/CustomersContext';

export default function CustomersHeader() {
  const { state, openWhatsAppBlast } = useCustomers();
  ...
  const activeNow = state.customers.filter((c) => {
    ...
    const daysSinceLastPurchase = Math.floor(
      (new Date().getTime() - new Date(c.lastPurchase.created_at).getTime()) / (1000 * 60 * 60 * 24)
    );
  ...
  return (
    <div className="mb-6">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 font-poppins">Customers</h1>
          <p className="text-gray-600 font-inter">Manage your customer base and relationships</p>
        </div>
          ...
          return (
            <div key={index} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600 font-inter">{card.title}</p>
                  ...
                  <div className="flex items-center mt-2">
                    ...
                    <span className="text-sm text-gray-500 ml-1 font-inter">vs last month</span>
                  </div>
                </div>
