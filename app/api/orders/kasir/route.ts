...
export async function POST(request: Request) {
  try {
    ...
    // Backend expects: { product_id, jumlah, harga_beli, note }
    ...
    console.log(
      "[Kasir Orders API] Sending to backend:",
    ...
    try {
      ...
      console.error(
        "[Kasir Orders API] Failed to parse response:",
        responseText
      );
      throw new Error(
        `Invalid JSON response from backend: ${responseText.substring(0, 200)}`
      );
    }
