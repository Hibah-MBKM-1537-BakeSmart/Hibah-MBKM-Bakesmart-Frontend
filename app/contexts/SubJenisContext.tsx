...
interface SubJenisContextType {
  ...
  updateSubJenis: (id: number, data: Partial<Omit<SubJenis, "id">>) => Promise<SubJenis | null>;
...
const SubJenisContext = createContext<SubJenisContextType | undefined>(undefined);
...
export function SubJenisProvider({ children }: { children: ReactNode }) {
  ...
  const fetchSubJenisFromProducts = async (): Promise<SubJenis[]> => {
    try {
      console.log("[SubJenisContext] Fetching sub_jenis from products (fallback)...");
      ...
      interface ProductJenis { id: number; nama_id: string; nama_en: string; }
      interface ProductSubJenis { id: number; nama_id: string; nama_en: string; jenis_id?: number; ref_jenis_id?: number; }
        ...
        if (Array.isArray(product.sub_jenis)) {
          product.sub_jenis.forEach((sj) => {
            if (!subJenisMap.has(sj.id)) {
              ...
              if (jenisId === 0 && Array.isArray(product.jenis) && product.jenis.length > 0) {
      ...
      console.log("[SubJenisContext] Extracted sub_jenis from products:", extractedSubJenis);
  ...
  const fetchSubJenis = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || errorData.error || `Failed to fetch sub_jenis (${response.status})`);
      }
      ...
      if (result.backendError || (Array.isArray(data) && data.length === 0 && result.backendError)) {
      ...
      const mappedData: SubJenis[] = Array.isArray(data) ? data.map((item: Record<string, unknown>) => ({
        id: Number(item.id),
      ...
      if (fallbackData.length > 0) {
        ...
        setError(err instanceof Error ? err.message : "Failed to fetch sub_jenis");
        setSubJenisList([]);
      }
    } finally {
      setLoading(false);
  ...
  // Get sub_jenis by ID
  const getSubJenisById = useCallback(async (id: number): Promise<SubJenis | null> => {
    try {
  ...
  const getSubJenisByJenisId = useCallback((jenisId: number): SubJenis[] => {
    return subJenisList.filter(sj => sj.jenis_id === jenisId);
  }, [subJenisList]);

  // Create new sub_jenis
  const createSubJenis = useCallback(async (data: Omit<SubJenis, "id">): Promise<SubJenis | null> => {
    try {
      const response = await fetch("/api/sub_jenis", {
        method: "POST",
      ...
      setError(err instanceof Error ? err.message : "Failed to create sub_jenis");
      return null;
    }
  }, []);

  // Update sub_jenis
  const updateSubJenis = useCallback(async (id: number, data: Partial<Omit<SubJenis, "id">>): Promise<SubJenis | null> => {
    try {
      const response = await fetch(`/api/sub_jenis/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
      ...
      // Update local state
      setSubJenisList(prev => prev.map(sj => sj.id === id ? updatedSubJenis : sj));
      return updatedSubJenis;
    } catch (err) {
      console.error("Error updating sub_jenis:", err);
      setError(err instanceof Error ? err.message : "Failed to update sub_jenis");
      return null;
    }
  }, []);
  ...
  const deleteSubJenis = useCallback(async (id: number): Promise<boolean> => {
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to delete sub_jenis");
      return false;
    }
  }, []);

  // Add sub_jenis to product
  const addSubJenisToProduct = useCallback(async (productId: number, subJenisId: number): Promise<boolean> => {
    try {
      console.log(`[SubJenisContext] Adding sub_jenis ${subJenisId} to product ${productId}`);
      const response = await fetch(`/api/products/${productId}/sub_jenis/${subJenisId}`, {
        method: "POST",
      });
      
      if (response.status === 409) {
        // Already exists - not an error
        console.log("[SubJenisContext] Sub_jenis already assigned to product");
        return true;
      }
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || "Failed to add sub_jenis to product");
      }
      
      console.log("[SubJenisContext] Successfully added sub_jenis to product");
      return true;
    } catch (err) {
      console.error("Error adding sub_jenis to product:", err);
      setError(err instanceof Error ? err.message : "Failed to add sub_jenis to product");
      return false;
    }
  }, []);

  // Remove sub_jenis from product
  const removeSubJenisFromProduct = useCallback(async (productId: number, subJenisId: number): Promise<boolean> => {
    try {
      console.log(`[SubJenisContext] Removing sub_jenis ${subJenisId} from product ${productId}`);
      const response = await fetch(`/api/products/${productId}/sub_jenis/${subJenisId}`, {
        method: "DELETE",
      });
      ...
      if (!response.ok) {
        if (response.status === 404) {
          ...
          console.log("[SubJenisContext] Sub_jenis not found on product (may already be removed)");
          return true;
        }
        const errorData = await response.json().catch(() => ({}));
        throw new Error(errorData.message || "Failed to remove sub_jenis from product");
      }
      
      console.log("[SubJenisContext] Successfully removed sub_jenis from product");
      return true;
    } catch (err) {
      console.error("Error removing sub_jenis from product:", err);
      setError(err instanceof Error ? err.message : "Failed to remove sub_jenis from product");
      return false;
    }
  }, []);
